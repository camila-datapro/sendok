...
$(document).ready(function () {
	...
	$.ajax({
		...
		var productos = msg.prod;
		var templis = msg.temp;
		var counter = 2;
		var opcionestemp = "<option value = '0' selectect>Seleccione un tipo de documento</option>";
		for (var i = 0; i < templis.length; i++) {
			opcionestemp = opcionestemp + '<option value="' + templis[i].id + '">' + templis[i].nombretemplate + '</option>';
		}
		$("#tipo_documento").append(opcionestemp);
...
function guardarPropuesta() {
	const elemento = document.getElementById('propuesta_documento');
	...
	$.ajax({
		...
		html2pdf()
			.set({
				...
				html2canvas: {
					compress: true,
					scale: 0.8, // a mayor escala, mejores graficos pero mas peso
				},
				jsPDF: {
					compress: true,
					unit: "in",
...
function vistaPreviaPDF() {
	...
	if (msg_info == "") {
		...
		$.ajax({
			...
			data: {
				idtemp: $("#tipo_documento").val(),
				idcliente: id_cliente,
				_token: $('input[name="_token"]').val()
			} //esto es necesario, por la validacion de seguridad de laravel
		}).done(function (msg) {

			if (!msg.prop[0].numero_folio) {
				msg.prop[0].numero_folio = 0;
			}
			$("#propuesta_documento").html(msg.temp);

			if (msg.tipo == 0) {
				var folio_propuesta = "PC" + id_cliente + "_" + (parseInt(msg.prop[0].numero_folio) + 1);
			...
			if (msg.tipo == 1) {
				...
				var _clientId = $("#select_cliente option:selected").attr('id');

				var randomID = IDGenerate();
				var folio_propuesta = "PC" + _clientId + "_" + randomID;
				$("#folio_propuesta").text(folio_propuesta + "-1");


				var cadena = msg.temp;
				var nuevaCadena = cadena.replace(/%nombreempresa%/gi, msg.cliente[0].nombre_cliente);
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/%rutempresa%/gi, msg.cliente[0].rut_cliente);
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/%regionempresa%/gi, msg.cliente[0].region);
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/%comunaempresa%/gi, msg.cliente[0].comuna);
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/%direccionempresa%/gi, msg.cliente[0].direccion_cliente);
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/%nombrecontacto%/gi, msg.cliente[0].nombre_contacto);
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/%apellidocontacto%/gi, msg.cliente[0].apellido_contacto);
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/%cargocontacto%/gi, msg.cliente[0].cargo_contacto);
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/%emailcontacto%/gi, msg.cliente[0].email_cliente);
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/%telefonocontacto%/gi, msg.cliente[0].fono_cliente);
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/Agregar Fecha/gi, fechatabla());
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/Nombre Usuario/gi, msg.nameuser);
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/Email Usuario/gi, msg.emailuser);
				cadena = nuevaCadena;
				nuevaCadena = cadena.replace(/Teléfono Usuario/gi, msg.fonouser);

				nuevaCadena = cadena.replace('<em>', '');
				nuevaCadena = cadena.replace('</em>', '');

				$("#id_usuario").text(msg.iduser);
				...
				nuevaCadena = cadena.replace(/%ProductGrid%/gi, $("#tabla_propuesta_container").html());
				cadena = nuevaCadena;
				$("#propuesta_documento").html(cadena);
